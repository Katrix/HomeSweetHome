package net.katsstuff.homesweethome.internal
...
/**
  ...
  */
class MutableNestedMap[A, B, C] private (internalMap: mutable.Map[A, mutable.Map[B, C]], create: () => mutable.Map[B, C])
...
object MutableNestedMap {
  def apply[A, B, C](
      internalMap: mutable.Map[A, mutable.Map[B, C]],
      create: () => mutable.Map[B, C]
  ): MutableNestedMap[A, B, C] = new MutableNestedMap(internalMap, create)
}
